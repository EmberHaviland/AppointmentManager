...
namespace AppointmentManager.Azure;
...
public class CosmosDbWrapper
{
    ...
    public CosmosDbWrapper(IConfiguration configuration)
    {
        ...
        string cosmosEndpoint = configuration["AppointmentManager:ConnectionStrings:CosmosEndpoint"]!;
        string? cosmosConnectionString = configuration["AppointmentManager:ConnectionStrings:CosmosConnectionString"];
        ...
        string databaseName = configuration["AppointmentManager:ConnectionStrings:CosmosDatabaseName"]!;
        string containerName = configuration["AppointmentManager:ConnectionStrings:CosmosContainerName"]!;
    ...
    public async Task DeleteItemAsync(string id, string pk)
    {
        using (var log = _logger.StartMethod(nameof(DeleteItemAsync)))
        {
            log.SetAttribute("id", id);
            log.SetAttribute("pk", pk);
            await _container.DeleteItemAsync<AppointmentData>(id, new PartitionKey(pk));
        }
    }
}